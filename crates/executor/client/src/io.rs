...
///
/// The default serde representation of [`Block`] is not bincode-compatible (the inner
/// transaction envelope serializes via `#[serde(flatten)]`, which makes bincode fail with
/// `SequenceMustHaveLength`). reth makes blocks bincode-serializable by round-tripping through
/// RLP bytes; we mirror that here because the previous
